...
## format specifies the datatype of the text
    ...
    def _clean_up_metadata(self):
        ...
        for key in change_to_second_value:
            try:
                self.original_metadata["experimental setup"][key] = self.original_metadata["experimental setup"][key]["2"]
            except KeyError:
                pass
        ...
        for key, value in self.original_metadata["experimental setup"].items():
            if isinstance(value, dict):
                self.original_metadata["experimental setup"][key] = self.original_metadata["experimental setup"][key]["1"]

        for key, value in self.original_metadata["date"].items():
            if isinstance(value, dict):
                self.original_metadata["date"][key] = self.original_metadata["date"][key]["1"]

        for key, value in self.original_metadata["file information"].items():
            if isinstance(value, dict):
                self.original_metadata["file information"][key] = self.original_metadata["file information"][key]["1"]

        ## convert strings to float
        for key in convert_to_numeric:
            try:
                self.original_metadata["experimental setup"][key] = float(self.original_metadata["experimental setup"][key])
            except KeyError:
                pass
        ...
        try:
            self.original_metadata["experimental setup"][new_grating_key_name] = self.original_metadata["experimental setup"]['Grating']
        ...
        try:
            self.original_metadata["experimental setup"][new_filter_key_name] = self.original_metadata["experimental setup"]["ND Filter"]
    ...
    def get_original_metadata(self):
        ...
        self.original_metadata["experimental setup"]["measurement_type"] = self.measurement_type
        ...
        if array.size >= 3:
            ...
            if not np.isclose(min_array, 0):
                ...
                if rel_diff_compare > 0.01 and self.use_uniform_wavelength_axis:
                    _logger.warning(f"The relative variation of the {name}-axis-scale ({rel_diff_compare}) is greater than 1%. Using a non-uniform-axis is recommended.")
            if not np.isclose(abs_diff_compare, 0) and self.use_uniform_wavelength_axis:
                _logger.warning(f"The difference between consecutive entrys of the {name}-axis-scale varies ({abs_diff_compare} between first 2 and last 2). Consider using a non-uniform-axis.")
    ...
    def _set_signal_type(self, xml_element):
        ...
        for child in xml_element:
            ...
            if id == '0x7C696E75':
                self.original_metadata["experimental setup"]["signal units"] = child.text
    ...
    def get_data(self):
        ...
        if num_rows == 1:
            ...
            if self.has_nav2:
                if self.has_nav1:
                    self.sig_array = np.reshape(self.sig_array, (self.nav2_size, self.nav1_size, num_cols))
                else:
                    self.sig_array = np.reshape(self.sig_array, (self.nav2_size, num_cols))
            elif self.has_nav1 and not self.has_nav2:
    ...
    def map_metadata(self):
        ...
        self._set_metadata(self.metadata["General"], "notes",
                           self.original_metadata["file information"], "Remark")
        try:
        ...
        try:
            ...
            intensity_units = self.original_metadata["experimental setup"]["signal units"]
        except KeyError:
            pass
        else:
            ...
            self.metadata["Signal"]["quantity"] = f"{intensity_axis} ({intensity_units})"
            self.metadata["Signal"]["signal_type"] = self.signal_type

        self._set_metadata(self.metadata["Sample"], "description",
                           self.original_metadata["file information"], "Sample")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Laser"], "wavelength (nm)",
                           self.original_metadata["experimental setup"], "Laser (nm)")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Laser"], "objective_magnification",
                           self.original_metadata["experimental setup"], "Objective")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Laser"]["Filter"], "optical_density",
                           self.original_metadata["experimental setup"], "ND Filter (%)")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Spectrometer"], "central_wavelength (nm)",
                           self.original_metadata["experimental setup"], "Spectro (nm)")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Spectrometer"], "model",
                           self.original_metadata["experimental setup"], "Instrument")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Spectrometer"]["Grating"], "groove_density",
                           self.original_metadata["experimental setup"], "Grating (gr/mm)")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Spectrometer"], "entrance_slit_width",
                           self.original_metadata["experimental setup"], "Hole")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Spectrometer"], "spectral_range",
                           self.original_metadata["experimental setup"], "Range")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"], "model",
                           self.original_metadata["experimental setup"], "Detector")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"], "binning",
                           self.original_metadata["experimental setup"], "Binning")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"], "temperature (°C)",
                           self.original_metadata["experimental setup"], "Detector temperature (°C)")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"], "exposure_per_frame (s)",
                           self.original_metadata["experimental setup"], "Acq. time (s)")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"], "frames",
                           self.original_metadata["experimental setup"], "Accumulations")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "Autofocus",
                           self.original_metadata["experimental setup"], "Autofocus")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "SWIFT",
                           self.original_metadata["experimental setup"], "SWIFT")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "AutoExposure",
                           self.original_metadata["experimental setup"], "AutoExposure")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "Spike filter",
                           self.original_metadata["experimental setup"], "Spike filter")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "DeNoise",
                           self.original_metadata["experimental setup"], "DeNoise")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "ICS correction",
                           self.original_metadata["experimental setup"], "ICS correction")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "Dark correction",
                           self.original_metadata["experimental setup"], "Dark correction")
        self._set_metadata(self.metadata["Acquisition_instrument"]["Detector"]["processing"], "Inst. Process",
                           self.original_metadata["experimental setup"], "Inst. Process")

        ## calculate and set integration time
        try:
            integration_time = (self.original_metadata["experimental setup"]["Accumulations"]
                                * self.original_metadata["experimental setup"]["Acq. time (s)"])
        except KeyError:
            pass
        else:
            self.metadata["Acquisition_instrument"]["Detector"]["integration_time (s)"] = integration_time

        ## convert filter range from percentage (0-100) to (0-1)
        try:
            self.metadata["Acquisition_instrument"]["Laser"]["Filter"]["optical_density"] /= 100
        except KeyError:
            pass
...
def file_reader(file_path, lazy=False, use_uniform_wavelength_axis=True, **kwds):
    ...
    jy = JobinYvonXMLReader(file_path, use_uniform_wavelength_axis=use_uniform_wavelength_axis)
    jy.parse_file()
    jy.get_original_metadata()
    jy.get_axes()
    jy.get_data()
    jy.map_metadata()
    dictionary = {'data': jy.sig_array,
                  'axes': jy.axes,
